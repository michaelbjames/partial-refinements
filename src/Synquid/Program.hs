...
renameAsImpl :: (Id -> Bool) -> Program t -> RType -> RType
...
addPositiveGuard fmls = subtypeGuards %~ (first (Set.union $ Set.fromList fmls))

addNegativeGuard :: [Formula] -> Environment -> Environment
addNegativeGuard fmls = subtypeGuards %~ (second (Set.union $ Set.fromList fmls))
