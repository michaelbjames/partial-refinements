...
greatestFixPoint quals extractAssumptions candidates = do
  ...
  where
    ...
    debugOutput cands cand inv modified =
      writeLog 3 (text "[greatestFixPoint]:" <> vsep [
        nest 2 $ text "Candidates" <+> parens (pretty $ length cands) $+$ (vsep $ map pretty cands),
...
strengthen qmap extractAssumptions fml@(Binary Implies lhs rhs) sol = do
    ...
    pruned <- ifM (asks semanticPrune)
      (ifM (asks agressivePrune)
        (do
          ...
          writeLog 3 (text "[strengthen]: Pruned valuations:" $+$ vsep (map pretty valuations'))
    ...
    writeLog 3 (text "[strengthen]: Diffs:" <+> parens (pretty $ length pruned) $+$ vsep (map pretty pruned))
    return pruned
  where
...
leastFixPoint extractAssumptions (cand@(Candidate sol _ _ _):rest) = do
    ...
    case solMb' of
      Nothing -> do
                  writeLog 3 (text "[leastFixPoint]: All constraints:" $+$ vsep (map pretty (Set.toList $ validConstraints cand `Set.union` invalidConstraints cand)))
  ...
  where
    ...
    debugOutput cand inv modified =
      writeLog 3 (text "[leastFixPoint]:" <> vsep [
