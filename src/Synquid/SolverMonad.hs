...
class (Monad s, Applicative s, MonadFail s) => MonadHorn s where
  ...
  checkCandidates :: SolverCallType -> [Formula] -> ExtractAssumptions ->[Candidate] -> s [Candidate]   -- ^ Check validity or consistency of constraints under current candidates
  refineCandidates :: [(Formula, String)] -> QMap -> ExtractAssumptions -> [Candidate] -> s [Candidate] -- ^ Refine current candidates to satisfy new constraints
